...
\item Une caméra (classe \texttt{Camera}) permettant de choisir le type de projection (perspective et orthographique) définie par sa position et la direction visée. De plus différents paramètres permettent de modéliser l'optique de la caméra. Cette caméra transforme les coordonnées 3D en coordonnées 2D à l'aide d'une projection.
...
les indices des sommets que l'on désire relier. Par exemple, BO=[1,4,5,10,1] signifie que l'on veut relier le sommet numéro 1 au 4, le 4 au 5, le 5 au 10, et le 10 au 1. 
% balise LATEX pour n° ?
 L affichage peut être de plusieurs types : points,lignes, ou lignes  brisées, triangles (OpenGL utilise que des triangles).\\
 
 
 
 Notre programme Vertigo, utilisera ces VBO pour le rendu des objets graphiques sous OpenGL.
...
Le \texttt{Renderer} va permettre de générer l'image et le rendu, à partir du graphe de scène. Ce dernier est en communication avec la scène et envoie des données à la carte graphique sous forme de BO qui sont stockés sur la VRAM \footnote{RAM de la carte graphique}. \\
Le code OpenGL sera localisé uniquement dans la partie moteur de rendu (\textbf{package graphics.jogl} et \textbf{graphics.lwjgl}). \\
Le \texttt{Renderer} est également chargé d'appliquer certains procédés comme des effets de transparences, d'ombre etc. et dans notre cas plus particulièrement les \texttt{Shader}.
...
Trois parties peuvent être distinguées  : % figure du renderer à mettre
\begin{itemize}
\item Initialisation et envoi des BO vers la carte graphique via le moteur de rendu.
\item Chargement des \texttt{Shader} (écrit en langage GLSL).
...
Ce dernier aura accès aux classes \texttt{Vertigo\_Viewer} et  \texttt{Shape} ainsi qu'à leur constructeur respectif.
...
Pour l'instant : 
\textbf{Pour la classe Vertigo\_Viewer (héritant de la classe PlugIn d'imageJ) :}
...
\subsection{Implantation du graphe de scène}
...
ShaderUtils : classes utilitaires pour le chargement et compilation des \textit{shaders} (\textit{vertex} et \textit{fragment shaders}).
...
\item Initialiser les \texttt{Shader}. % c'est ce qui transforme les points en pixels
\end{itemize}
...
% Soit on laisse en portait et l image et tout petite et coupée...
 ...
 Avant de le connecter au logiciel ImageJ, nous afficherons tout d'abord un cube, de différentes manières (en commençant par les sommets, puis en rajoutant des indexes, une texture, etc.). Puis différents objets simple tel que :
 une pyramide, une sphère etc. Et enfin, différents objets expérimentaux issus du laboratoire. 
