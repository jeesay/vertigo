...
Le \textit{renderer} va permettre de générer l'image, le rendu, à partir du graphe de scène. Le renderer est en communication avec la scène et envoie des données à la carte graphique sous forme de VBO qui sont stockés sur la VRAM. Ce n'est donc que dans cette partie que le code open GL se retrouvera.
Le renderer est également chargé d'appliquer certains procédés comme des effets de transparences, d'ombre (etc.) et dans notre cas plus particulièrement les \textit{shaders}.
